...
const BASE_URL = 'https://www.axess.com.tr';
const LISTING_URL = `${BASE_URL}/axess/kampanyalar`;

const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

async function runCrawler() {
  console.log('=== Akbank Axess Derinlemesine Scraper Başlatılıyor ===');
  let browser;
  const campaigns = [];
  ...
  try {
    ...
    console.log('Axess listeleme sayfasına gidiliyor...');
    ...
    // Linkleri ayıklayalım
    const links = await page.evaluate(() => {
      const anchors = Array.from(document.querySelectorAll('a'));
      return anchors
        .map(a => a.getAttribute('href'))
        .filter(href => href && (href.includes('/kampanyalar/') || href.includes('/kampanya/')))
        .map(href => href.startsWith('http') ? href : `https://www.axess.com.tr${href}`);
    });

    const uniqueLinks = [...new Set(links)];
    const totalCount = uniqueLinks.length;
    console.log(`\n🎉 Toplam benzersiz Axess kampanyası tespit edildi: ${totalCount}`);
    ...
    for (let i = 0; i < totalCount; i += batchSize) {
      ...
      console.log(`   [Axess] Detay Paketi Okunuyor (${i + 1}-${Math.min(i + batchSize, totalCount)} / ${totalCount})...`);
        ...
        try {
          ...
          const detail = await detailPage.evaluate((url) => {
            ...
            return {
              ...
              conditions: conditions.length > 0 ? conditions : ['Detaylı bilgi Axess mobil uygulamasında yer almaktadır.'],
              bank: 'Akbank Axess'
            };
          }, url);
          ...
          console.error(`      [Axess] Detay okuma hatası (${url}):`, err.message);
          return null;
        } finally {
    ...
    console.warn(`[Axess Scraper] Canlı tarama sırasında hata/engel oluştu: ${error.message}`);
    console.log('[Axess Scraper] Gerçek kampanya bulunamadığı için boş dizi kaydediliyor.');
  } finally {
    if (browser) await browser.close();
    const outputPath = path.join(__dirname, 'campaigns-axess.json');
    fs.writeFileSync(outputPath, JSON.stringify(campaigns, null, 2), 'utf-8');
    console.log(`[Axess Scraper] Tamamlandı. ${campaigns.length} kampanya kaydedildi.`);
  }
}
