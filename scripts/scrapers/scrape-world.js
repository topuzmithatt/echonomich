...
async function runCrawler() {
  console.log('=== Yapı Kredi World Derinlemesine Scraper Başlatılıyor ===');
  let browser;
  const campaigns = [];
  ...
  try {
    ...
    console.log('World listeleme sayfasına gidiliyor...');
    ...
    const links = await page.evaluate(() => {
      ...
      return anchors
        ...
        .map(href => href.startsWith('http') ? href : `https://www.worldcard.com.tr${href}`);
    });

    const uniqueLinks = [...new Set(links)];
    const totalCount = uniqueLinks.length;
    console.log(`\n🎉 Toplam benzersiz World kampanyası tespit edildi: ${totalCount}`);
    ...
    for (let i = 0; i < totalCount; i += batchSize) {
      ...
      console.log(`   [World] Detay Paketi Okunuyor (${i + 1}-${Math.min(i + batchSize, totalCount)} / ${totalCount})...`);
        ...
        try {
          ...
          const detail = await detailPage.evaluate((url) => {
            ...
            return {
              ...
              conditions: conditions.length > 0 ? conditions : ['Detaylı bilgi World mobil uygulamasında yer almaktadır.'],
          ...
          console.error(`      [World] Detay okuma hatası (${url}):`, err.message);
          return null;
        } finally {
    ...
    console.warn(`[World Scraper] Canlı tarama sırasında hata/engel oluştu: ${error.message}`);
    console.log('[World Scraper] Gerçek kampanya bulunamadığı için boş dizi kaydediliyor.');
  } finally {
    if (browser) await browser.close();
    const outputPath = path.join(__dirname, 'campaigns-world.json');
    fs.writeFileSync(outputPath, JSON.stringify(campaigns, null, 2), 'utf-8');
    console.log(`[World Scraper] Tamamlandı. ${campaigns.length} kampanya kaydedildi.`);
  }
}
