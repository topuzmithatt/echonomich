...
async function runCrawler() {
  console.log('=== İş Bankası Maximum Derinlemesine Scraper Başlatılıyor ===');
  let browser;
  const campaigns = [];
  ...
  try {
    ...
    console.log('Maximum listeleme sayfasına gidiliyor...');
    ...
    const links = await page.evaluate(() => {
      ...
      return anchors
        ...
        .map(href => href.startsWith('http') ? href : `https://www.maximum.com.tr${href}`);
    });

    const uniqueLinks = [...new Set(links)];
    const totalCount = uniqueLinks.length;
    console.log(`\n🎉 Toplam benzersiz Maximum kampanyası tespit edildi: ${totalCount}`);
    ...
    for (let i = 0; i < totalCount; i += batchSize) {
      ...
      console.log(`   [Maximum] Detay Paketi Okunuyor (${i + 1}-${Math.min(i + batchSize, totalCount)} / ${totalCount})...`);
        ...
        try {
          ...
          const detail = await detailPage.evaluate((url) => {
            ...
            return {
              ...
              conditions: conditions.length > 0 ? conditions : ['Detaylı bilgi Maximum mobil uygulamasında yer almaktadır.'],
          ...
          console.error(`      [Maximum] Detay okuma hatası (${url}):`, err.message);
          return null;
        } finally {
    ...
    console.warn(`[Maximum Scraper] Canlı tarama sırasında hata/engel oluştu: ${error.message}`);
    console.log('[Maximum Scraper] Gerçek kampanya bulunamadığı için boş dizi kaydediliyor.');
  } finally {
    if (browser) await browser.close();
    const outputPath = path.join(__dirname, 'campaigns-maximum.json');
    fs.writeFileSync(outputPath, JSON.stringify(campaigns, null, 2), 'utf-8');
    console.log(`[Maximum Scraper] Tamamlandı. ${campaigns.length} kampanya kaydedildi.`);
  }
}
